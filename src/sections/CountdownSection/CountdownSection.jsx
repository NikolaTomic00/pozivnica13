...
export default function CountdownSection() {
  const [timeLeft, setTimeLeft] = useState(() => Math.max(0, weddingDate.getTime() - Date.now()));
  ...
  return (
    <section className="flex items-center py-8 text-center sm:py-12">
      <div className="flex w-full flex-col items-center bg-[var(--sand-deep)] px-4 py-12 text-[var(--cream)] sm:px-10 sm:py-16">
        <p className="text-xs uppercase tracking-[.4em]">Do našeg velikog dana</p>
        <div className="mt-10 grid w-full max-w-2xl grid-cols-4 gap-3 sm:gap-8">
          {countdown.map(([number, label]) => (
            <div key={label}>
              <div className="font-heading text-4xl sm:text-6xl">{String(number).padStart(2, "0")}</div>
              <div className="mt-2 text-[10px] uppercase tracking-[.2em] opacity-75 sm:text-xs">{label}</div>
            </div>
          ))}
        </div>
      </div>
